...
PROFILE = 'automq-oci'
COMPARTMENT = 'ocid1.tenancy.oc1..aaaaaaaal4wmmpzv2fzkdz2vrfdizywgzjid6dqlgcankrrr7jyydo7ozb3a'
NAMESPACE = 'fryovegrk8e7'
...
def prior_ids():
    ...
    for path in Path(__file__).parent.glob('resources-*.json'):
        ...
        if snapshot.get('profile') != PROFILE or snapshot.get('compartment') != COMPARTMENT:
            continue
        for key in found:
...
def inventory():
    ...
    result = {'profile': PROFILE, 'compartment': COMPARTMENT, 'namespace': NAMESPACE, 'observed_at': datetime.now(timezone.utc).isoformat()}
    ...
    subnet = oci('network', 'subnet', 'get', '--subnet-id',
                 'ocid1.subnet.oc1.eu-frankfurt-1.aaaaaaaaotya32pihejgi25vrdfnjda3qg52kpsjnd7od5oiqifbsi4rqqma')
