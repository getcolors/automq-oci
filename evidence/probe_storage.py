...
for operation in ('head_bucket', 'list_objects_v2', 'get_object'):
    try:
        getattr(s3, operation)(Bucket=opts['state'], **({'Key': '_colors/backend-owner.json'} if operation == 'get_object' else {}))
...
try:
    with store._oci_request(s3, 'HEAD', opts['state'], '_colors/backend-owner.json'):
        raise AssertionError('application API signing key can access state bucket')
except HTTPError as exc:
    checks['native_state_head_http'] = exc.code
...
def main():
    ...
    parser.add_argument('--namespace', default='frqagv4ppzaz')
    parser.add_argument('--endpoint', default='https://frqagv4ppzaz.compat.objectstorage.eu-frankfurt-1.oraclecloud.com')
    for role in ('state', 'data', 'ops'):
        parser.add_argument('--' + role, default=f'automq-oci-{role}-frqagv4ppzaz')
    ...
    for kind in ('bucket', 'object'):
        command = ['oci', 'os', kind, 'get' if kind == 'bucket' else 'head',
                   ...
                   '--region', args.region, '--auth', 'security_token']
        if kind == 'object':
